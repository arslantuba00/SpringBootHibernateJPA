...
public class KonuService
{
	...
	{
		return konuRepo.findById(id).get();//getById kullanılamıyor!
	}

	public Konu findByName(String isim)//hoca metod ismini find yazdı.
	{
	...
	{
		return konuRepo.findKonuByNAMELike("%" + expression + "%", Sort.by(Order.desc("ID")));
